...
window.addEventListener("keydown", event => {
  if ((event.keyCode == '38') && (!gameOver)) {
  	// console.log("Detect move up")
    // up arrow
    move('forward');

  }
  else if ((event.keyCode == '40') && (!gameOver)) {
  	// console.log("Detect move down")
    // down arrow
    move('backward');

  }
  else if ((event.keyCode == '37') && (!gameOver)) {
  	// console.log("Detect move left")
    // left arrow
    move('left');
  }
    ...
    paused = !paused;
    console.log("pause or resume the game")
  }
});
...
const imgU = document.getElementById("imgURL");
...
function animate(timestamp) {
  ...
  if (stepStartTimestamp) {
    ...
    if (moveDeltaTime > stepTime && !gameOver) {
      switch (moves[0]) {
        case 'forward': {
          currentLane++;
